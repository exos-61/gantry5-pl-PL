...
class cbCalendars
{
	...
	public function __construct( $ui, $calendarType = null, $dateFormat = null, $timeFormat = null )
	{
		...
		$this->defDateFormat			=	( $dateFormat !== null ? $dateFormat : ( CBTxt::T( 'UE_DATE_FORMAT', '' ) != '' ? CBTxt::T( 'UE_DATE_FORMAT', '' ) : $ueConfig['date_format'] ) );
	...
	public function cbAddCalendar( $name, $label = null, $required = false, $value = null, $readOnly = false, $showTime = false, $minYear = null, $maxYear = null, $attributes = null, $serverTimeOffset = true, $offsetOverride = null )
	{
		...
		if ( $readOnly ) {
			...
			$return							=	'<input type="hidden" name="' . htmlspecialchars( $name ) . '" id="' . htmlspecialchars( $inputId ) . '" value="' . htmlspecialchars( $value ) . '" class="cbDatePicker cbValidationAllowed"' . ( trim( $attributes ) ? ' ' . $attributes : null ) . ' />'
											.	'<input type="text" id="' . htmlspecialchars( $inputId ) . 'Selector" value="' . htmlspecialchars( $offsetValue ) . '" class="cbDatePickerSelector form-control' . ( $required ? ' required' : null ) . ( $this->calendarType == 4 ? ' hidden' : null ) . '"' . ( $readOnly ? ' disabled="disabled"' : null ) . ( $label ? ' title="' . htmlspecialchars( $label ) . '"' : null ) . ' />'
