...
class Finder
{
	...
	public static function input( $language = 'default' )
	{
		...
		$return				=	'<div class="row no-gutters cbLanguageFinder">'
							...
							.		'</div>'
							.		'<div class="col-sm-8">'
							.			'<div class="cbLanguageFinderLoading spinner-border text-secondary hidden"></div>'
	...
	public static function find( $language, $search )
	{
		...
		// Cache the originally loaded language:
		...
		// Import the default language strings:
		CBTxt::import( $languagePath, 'default_language', 'language.php', false );
		CBTxt::import( $languagePath, 'default_language', 'admin_language.php', false );
		CBTxt::import( $languagePath, 'default_language', 'override.php', false, true );

		// Import the default plugin language strings:
		foreach ( $_PLUGINS->getLoadedPluginGroup( null ) as $plugin ) {
			CBTxt::import( $languagePath, 'default_language', 'cbplugin/' . $plugin->element . '-language.php', false );
			CBTxt::import( $languagePath, 'default_language', 'cbplugin/' . $plugin->element . '-admin_language.php', false );
		}

		// Grab all the default language keys and text (default_language gets converted to en-GB in setCurrentLanguage):
		$languageStrings			=	CBTxt::getStrings( 'en-GB' );

		// If the language we're searching isn't the default lets load it:
		if ( $language != 'default_language' ) {
			if ( file_exists( $languagePath . '/' . $language ) ) {
				...
				// Grab all the specific language keys and text then merge with default to ensure missing strings can still be found:
				$languageStrings	=	array_merge( $languageStrings, CBTxt::getStrings( $language ) );
			}
		}

		// Reset language back to original since import will change the language:
